...
SELECT
    -- AVG(COALESCE(g.grid_intersections, 0)/t.total_intersections)
    t.road,
...
WHERE
    t.total_intersections - COALESCE(g.grid_intersections, 0) > 5 AND
    ( (ABS(orientation_angle - 90) < 45) OR (ABS(orientation_angle + 90) < 45) );
